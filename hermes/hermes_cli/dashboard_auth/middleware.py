...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    if path.startswith("/api/") or path == "/agui" or path.startswith("/agui/"):
        # API / AG-UI routes never get redirects: the browser fetch() API would
...
def _auto_sso_response(request: Request) -> Response | None:
    ...
    """
    path = request.url.path
    # APIs / AG-UI never auto-redirect (see _unauth_response). Only document loads.
    if path.startswith("/api/") or path == "/agui" or path.startswith("/agui/"):
        return None
