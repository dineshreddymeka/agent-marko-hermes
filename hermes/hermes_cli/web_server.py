...
async def auth_middleware(request: Request, call_next):
    ...
    if getattr(request.app.state, "auth_required", False):
        return await call_next(request)
    path = request.url.path
    # Protect Marko AG-UI chat the same way as /api/* (one-hop, no Bun layer).
    needs_token = (
...
class MarkoSessionCreate(BaseModel):
    """Body for Agent-Marko session create (Marko expects POST /api/sessions)."""
...
def create_session_marko(body: MarkoSessionCreate):
...
def mount_spa(application: FastAPI):
    ...
    async def serve_spa(full_path: str, request: Request):
        ...
        if full_path == "agui" or full_path.startswith("agui/"):
            return JSONResponse(
                {"detail": f"No such AG-UI endpoint: /{full_path}"},
                status_code=404,
            )
...
# Agent-Marko AG-UI chat (POST /agui SSE) — in-process, no Bun middle layer.
from hermes_cli.agui_endpoint import router as _agui_router  # noqa: E402
app.include_router(_agui_router)

# Agent-Marko Memory panel (/api/memory/entries, /api/search?type=memory).
...
async def marko_boot(request: Request):
    """Loopback-only session token for Agent-Marko Vite proxy development.

    Production Marko SPA served by Hermes already receives
    ``window.__HERMES_SESSION_TOKEN__`` via index.html injection.
    """
    ...
    gated = bool(getattr(app.state, "auth_required", False))
    return {
        "token": None if gated else _SESSION_TOKEN,
        "authRequired": gated,
        "backend": "hermes",
        "agui": "/agui",
        "docs": "/docs",
